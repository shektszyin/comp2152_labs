...
def get_network_info():
    """Run ip addr show and return the output."""
    result = subprocess.run(
        ["ip", "addr", "show"],
        capture_output=True, text=True
    )
...
def get_arp_table():
    """Run ip neigh to show all devices on the local network."""
    result = subprocess.run(
        ["ip", "neigh"],
        capture_output=True, text=True
    )
...
def parse_ping(output):
    """Parse Linux ping output and extract key statistics."""
    ...
    for line in lines:
        # Linux stats: "3 packets transmitted, 3 received, 0% packet loss, time 2003ms"
        ...
        # Linux timing: "rtt min/avg/max/mdev = 11.5/12.3/13.1/0.654 ms"
...
def parse_mac_address(output):
    """Parse ip addr show output to extract MAC and IP address."""
    lines = output.strip().split("\n")
    info = {"mac": "Not found", "ip": "Not found"}

    for line in lines:
        line_stripped = line.strip()
        # Linux: "link/ether aa:bb:cc:dd:ee:ff brd ff:ff:ff:ff:ff:ff"
        if "link/ether" in line_stripped:
            parts = line_stripped.split()
            ether_index = parts.index("link/ether")
            if ether_index + 1 < len(parts):
                mac = parts[ether_index + 1]
                # Skip loopback (00:00:00:00:00:00) — take first real MAC
                if mac != "00:00:00:00:00:00" and info["mac"] == "Not found":
                    info["mac"] = mac
        # Linux: "inet 192.168.1.100/24 brd 192.168.1.255 scope global eth0"
        if "inet " in line_stripped and "inet6" not in line_stripped:
            ...
            if inet_index + 1 < len(parts):
                ...
                # Skip loopback 127.0.0.1 — take first real IP
                if ip != "127.0.0.1" and info["ip"] == "Not found":
                    info["ip"] = ip
...
def parse_arp_table(output):
    """Parse ip neigh output and return a list of devices."""
    lines = output.strip().split("\n")
    devices = []
...
def do_network_info():
    ...
    try:
        ...
        log_to_csv(LOG_FILE, "ip_addr", "show", net_data["mac"] + " / " + net_data["ip"], "Captured")
        print("Result logged.")
    except Exception as e:
        print("  Error: " + str(e))
        log_to_csv(LOG_FILE, "ip_addr", "show", "Error", "Failed")
...
def do_arp_table():
    ...
    try:
        ...
        log_to_csv(LOG_FILE, "ip_neigh", "local", str(len(devices)) + " devices", "Captured")
        print("\nResult logged.")
    except Exception as e:
        print("  Error: " + str(e))
...
#  TEST YOUR WORK
# ============================================================
# After completing Tasks 1-3, run the program:
main()
# ============================================================